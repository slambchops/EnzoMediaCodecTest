...
unsigned char ENZO_AVC_HEADERS_720P[VPU_SPS_SIZE + VPU_PPS_SIZE] =
		/* SPS */
		{ 0, 0, 0, 1, 103, 66, 64, 31, 166, 128, 80, 5, 185,
		/* PPS */
		  0, 0, 0, 1, 104, 206, 48, 164, 128};
...
static int decoder_open(struct decoder_info *dec, struct mediaBuffer *enc_src)
{
	...
	/*If no mediaBuffer was passed to init function, then try to init
	  the decoder at max resolution */
	info_msg("%s: Parsing input data\n", dec->decoder_name);
	if (enc_src == NULL) {
		if (dec->format == H264AVC) {
			info_msg("%s: AVC decoder defaulting to 720p...\n",
				 dec->decoder_name);
			struct mediaBuffer temp_src;
			temp_src.vBufOut = ENZO_AVC_HEADERS_720P;
			temp_src.bufOutSize = VPU_SPS_SIZE + VPU_PPS_SIZE;
			temp_src.dataSource = BUFFER;
		ret = dec_fill_bsbuffer(
				dec,
				dec->handle,
				&temp_src,
				dec->virt_bsbuf_addr,
			...
			err_msg("%s: no input for decoder to parse",
				dec->decoder_name);
			return DEC_ERROR;
		}
	} else {
...
static int decoder_allocate_framebuffer(struct decoder_info *dec)
{
	...
	dec->outbuf_cached = g2d_alloc(dec->bufwidth * dec->bufheight * 2, 1);//alloc physical contiguous memory for source image data with cacheable attribute
	if(!dec->outbuf_cached) {
		err_msg("%s:Failed to allocate cached output buffer!\n", dec->decoder_name);
		return DEC_ERROR;
	}
	...
	for (i = 0; i < totalfb; i++) {
		...
		pfbpool[i] = framebuf_alloc(vpu_fmt, color_space,
				    dec->bufwidth, dec->bufheight, mvCol);
		if (pfbpool[i] == NULL) {
	...
	bufinfo.maxDecFrmInfo.maxMbX = dec->bufwidth / 16;
	bufinfo.maxDecFrmInfo.maxMbY = dec->bufheight / 16;
	bufinfo.maxDecFrmInfo.maxMbNum = dec->bufwidth * dec->bufheight / 256;
...
static int decoder_parse(struct decoder_info *dec)
{
	...
	if (dec->format == H264AVC) {
		...
		/* Determine the color space of MJPG data*/
		if (initinfo.mjpg_sourceFormat == 0)
			dec->color_space = YUV420P;
		else if (initinfo.mjpg_sourceFormat == 1)
			dec->color_space = YUV422P;
		else if (initinfo.mjpg_sourceFormat == 2)
			dec->color_space = YUV224P;
		else if (initinfo.mjpg_sourceFormat == 3)
			dec->color_space = YUV444;
		else if (initinfo.mjpg_sourceFormat == 4)
			dec->color_space = YUV400;
	}
	...
	dec->bufwidth = ((initinfo.picWidth + 15) & ~15);
	align = 16;
	dec->bufheight = ((initinfo.picHeight + align - 1) & ~(align - 1));

	info_msg("%s: aligned resolution: %dx%d\n",dec->decoder_name,
		 dec->bufwidth, dec->bufheight);
			 ...
			 struct mediaBuffer *vid_dst)
{
	...
	while (param_change_loop < AVC_PARAM_LOOP_MAX) {
		...
		if (outinfo.decodingSuccess & 0x10) {
			warn_msg("%s: not enough bitstream to decode frame\n",
				 dec->decoder_name);
		...
		if (outinfo.indexFrameDisplay >= 0) {
			...
			/*If picType = 0, pic was I-frame. Otherwise P frame; */
			vid_dst->frameType = outinfo.picType;
		} else {
...
static void write_to_dst(struct decoder_info *dec,
			 struct mediaBuffer *vid_dst, int index)
{
	int height = (dec->bufheight + 15) & ~15 ;
	int stride = dec->bufwidth;
	int img_size;
	u8 *buf;
	...
	if (vid_dst->dataSource == FILE_SRC) {
		...
		s_buf.buf_paddr = (int)pfb->addrY;
		s_buf.buf_vaddr = buf;
		g2d_copy(g2d_handle, dec->outbuf_cached, &s_buf, img_size);
		g2d_finish(g2d_handle);
		g2d_close(g2d_handle);

		vid_dst->colorSpace = dec->color_space;
		vid_dst->dataSource = VPU_CODEC;
		vid_dst->bufOutSize = img_size;
		vid_dst->height = dec->bufheight;
		vid_dst->width = dec->bufwidth;
		vid_dst->imageHeight = dec->lastPicHeight;
		vid_dst->imageWidth = dec->lastPicWidth;
		vid_dst->vBufOut = (unsigned char *)dec->outbuf_cached->buf_vaddr;
		vid_dst->pBufOut = (unsigned char *)dec->outbuf_cached->buf_paddr;
	} else {
		vid_dst->colorSpace = dec->color_space;
		vid_dst->dataSource = VPU_CODEC;
		vid_dst->bufOutSize = img_size;
