...
static void SaveEncSliceInfo(u8 *SliceParaBuf, int size, struct nalInfoStruct *nalInfo, bool iFrame);
...
int vpu_encoder_encode_frame(struct encoder_info *enc, struct mediaBuffer *vid_src, struct mediaBuffer *enc_dst)
{
	...
	/* Now that we have an output frame from encoder, find the virtual
	   address so we can copy it to the destination buffer */
	...
	if (outinfo.picType == I_FRAME) {
		...
		SaveEncSliceInfo(outinfo.sliceInfo.addr, outinfo.sliceInfo.size,
				 &enc_dst->nalInfo, true);
		memcpy((void *)(enc->virt_outbuf_addr + VPU_SPS_SIZE + VPU_PPS_SIZE),
		       (void *)vbuf, outinfo.bitstreamSize);
		enc_dst->frameType = outinfo.picType;
		enc_dst->bufOutSize = outinfo.bitstreamSize + VPU_SPS_SIZE + VPU_PPS_SIZE;
		enc_dst->vBufOut = (unsigned char*)enc->virt_outbuf_addr;
		enc_dst->pBufOut = (unsigned char*)enc->phy_outbuf_addr;
	} else {
		SaveEncSliceInfo(outinfo.sliceInfo.addr, outinfo.sliceInfo.size,
				 &enc_dst->nalInfo, false);
...
static int encoder_open(struct encoder_info *enc)
{
	...
	encop.EncStdParam.avcParam.paraset_refresh_en = 1;
...
static void SaveEncSliceInfo(u8 *SliceParaBuf, int size, struct nalInfoStruct *nalInfo, bool iFrame)
{
	int i, nMbAddr, nSliceBits, startIndex;

	if (iFrame == true) {
		startIndex = 2;
		nalInfo->nalLength[0] = VPU_SPS_SIZE;
		nalInfo->nalLength[1] = VPU_PPS_SIZE;
	} else
		startIndex = 0;
	...
	for(i=0; i<size / 8; i++) {
		...
		SliceParaBuf += 8;
		nalInfo->nalLength[i + startIndex] = nSliceBits/8;
	}
	nalInfo->nalNumber = i + startIndex;
}
